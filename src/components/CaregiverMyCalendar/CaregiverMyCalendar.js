...
import styles from './CaregiverMyCalendar.styles';
...
class CaregiverMyCalendar extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    //this.mixpanel.track('View Caregivers My Calendar Home');
  }
  ...
  loadApiData() {
    ...
    PersonalCareCalendarApi.loadCalendar({ var: 'empty' })
      .promise.then(result => {
  ...
  onPressItem = item => {
    ...
    navigation.navigate('CaregiverCalendarDetails', { item });
  };
    ...
    return (
      <SafeAreaView>
        <View style={Globals.background}>
          <CustomHeaderBack onPressBack={this.onPressBack} title="My Scheduled Appointments" loading={actionLoading && !pullRefreshing} />
          <ScrollView
            refreshControl={
              <RefreshControl
